...
public class DetectionApi {
    ...
    ArrayList<ByFreqParams> byFreqParamsList = new ArrayList<>();
    ...
    class ByFreqParams
    {
        double minFrequency;
        double maxFrequency;
        double minStandardDeviation, maxStandardDeviation;
        int minNumZeroCross, maxNumZeroCross;
        int numRobust;
    }
	/**
    ...
    protected void addByFreqParams(ByFreqParams params)
    {
        byFreqParamsList.add(params);
    }
					...
					if (isPassedStandardDeviation(spectrogramData)){
						if (isPassedZeroCrossingRate(amplitudes)){
							if (isPassedFrequency(rangedSpectrum)){
                                if(LOG) System.out.println("isSpecificSound : true");
								return true;
							}
						}
					}
    ...
    public boolean isSpecificSound2(byte[] audioBytes) {
        ...
        if (numSamples > 0 && Integer.bitCount(numSamples) == 1) {
            ...
            if (frequencyUnitRange <= spectrum.length)
            {
                if(byFreqParamsList.size()>0)
                {
                    double robustFreq = getRobustFrequency(rangedSpectrum);
                    for(ByFreqParams params: byFreqParamsList)
                    {
                        if(robustFreq>params.minFrequency && robustFreq<= params.maxFrequency)
                        {
                            minStandardDeviation = params.minStandardDeviation;
                            maxStandardDeviation = params.maxStandardDeviation;
                            numRobust = params.numRobust;
                            maxNumZeroCross = params.maxNumZeroCross;
                            minNumZeroCross = params.minNumZeroCross;
                            break;
                        }
	...
	protected boolean isPassedStandardDeviation(double[][] spectrogramData){
		...
		if(LOG)  System.out.println("sd: " + sd + " " + result);
		return result;
	}
	...
	protected boolean isPassedFrequency(double[] spectrum){
		...
		double robustFrequency = getRobustFrequency(spectrum);
		...
		if(LOG)  System.out.println("freq: " + robustFrequency + " " + result);
		return result;
	}

    protected double getRobustFrequency(double[] spectrum)
    {
        // find the robust frequency
        ArrayRankDouble arrayRankDouble = new ArrayRankDouble();
	...
	protected boolean isPassedIntensity(double[] spectrum){
		...
		if(LOG)  System.out.println("intensity: " + intensity + " " + result);
	...
	protected boolean isPassedZeroCrossingRate(short[] amplitudes){
		...
		if(LOG)  System.out.println("zcr: " + numZeroCrosses + " " +result);
